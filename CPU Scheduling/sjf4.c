// we will be implementing non-preemptive SJF technique
#include <stdio.h>
#include <stdlib.h>
struct p{
...
int cmpfnc1(const void* a, const void* b){
  ...
  if(A->arrival == B->arrival){
    ...
    if(A->burst < B->burst) return -1;
    else if(A->burst == B->burst) return A->id > B->id ? 1 : -1;
  }
}
...
void gantt(struct p arr[], int n){
  ...
  for(int i=1; i < n;i++)
    for(int j = i; j < n-1; j++) if(arr[j].burst > arr[j+1].burst && arr[j+1].arrival <= arr[i-1].burst) swap(&arr[j], &arr[j+1]);
}
